...
Streamlit MVP. Совместим с model.py v2.2 (без параметра r_pbz).
M_pbz считается внутри app.py через трапециевидное интегрирование R(r).
"""
...
LANG = {
    "ru": {
        ...
        "btn_next": "Далее →", "btn_back": "← Назад", "btn_calc": "▶  Рассчитать",
        "btn_edit": "← Изменить параметры",
        ...
        "kpi_SI"     : "Максимальный SI (у забоя)",
        "kpi_R"      : "Макс. скорость R",
        "kpi_Mpbz"   : "Масса в ПЗП за период",
        "kpi_R_unit" : "кг/(м³·сут)",
        "kpi_M_unit" : "кг",
        "si_neg"     : "Недонасыщено",
        "si_low"     : "Слабый риск",
        ...
        "hmap_note" : "Центр — скважина. Цвет по R(r): синий (низкий) → красный (высокий).",
        "r_ax": "Радиус r (м)", "SI_ax": "SI(r)", "P_ax": "P (МПа)",
        "v_ax": "v (м/с)",     "R_ax": "R (кг/(м³·сут))",
        "sens_title": "Анализ чувствительности",
        "sens_note" : "Значение R у забоя (r = rw) при варьировании одного параметра.",
        ...
        "acc_y_pbz" : "Масса в ПЗП (кг)",
        "acc_y_all" : "Масса весь пласт (кг)",
        ...
        "det_SIe"   : "Базовый SI_e (пластовые условия)",
        "det_SIrw"  : "SI(rw) — максимум у забоя",
        "det_Rrw"   : "R(rw) — скорость у забоя",
        "det_Rmax"  : "R_max — максимум по профилю",
        "det_Mt"    : "M(t) весь пласт (кг)",
        "det_Mpbz"  : "M(t) призабойная зона (кг)",
        "det_eta"   : "η — доля осаждения Ca²⁺",
    },
    "en": {
        "app_title"   : "CaCO₃ Scale Predictor",
        ...
        "btn_next": "Next →", "btn_back": "← Back", "btn_calc": "▶  Calculate",
        "btn_edit": "← Edit parameters",
        ...
        "kpi_SI"     : "Max SI (at wellbore)",
        "kpi_R"      : "Max Deposition Rate R",
        ...
        "hmap_note" : "Center — wellbore. Color by R(r): blue (low) → red (high).",
        ...
        "acc_y_pbz" : "Near-wellbore mass (kg)",
        "acc_y_all" : "Total reservoir mass (kg)",
        ...
        "det_SIe"   : "Base SI_e (reservoir conditions)",
        "det_SIrw"  : "SI(rw) — max at wellbore",
        "det_Rrw"   : "R(rw) — rate at wellbore",
        "det_Rmax"  : "R_max — profile maximum",
        "det_Mt"    : "M(t) whole reservoir (kg)",
        "det_Mpbz"  : "M(t) near-wellbore zone (kg)",
        "det_eta"   : "η — Ca²⁺ deposition fraction",
    },
}
...
.kc-green::before  { background:#10b981; }
.kc-yellow::before { background:#f59e0b; }
...
div.stButton>button {
    background:#1a2640; color:#6c9cf5;
    border:1px solid #2d4a7a; border-radius:9px;
...
div.stButton>button[kind="primary"] {
    ...
    color:#93b8ff; border:1px solid #3b6ab8;
    font-size:0.95rem; padding:0.7rem 1.8rem;
}
...
def _calc_M_pbz(r_arr, R_arr, h, t, r_pbz, r_w):
    """Масса осадка в призабойной зоне [кг] — интеграл по r < r_pbz."""
    mask = r_arr <= max(r_pbz, r_w * 2)
    if mask.sum() < 2:
        # Fallback: оценка по первой точке
...
def _num(label, key, lo, hi, step=None, help_txt=None):
    val = st.number_input(
        label,
...
def _run_model_safe():
    """Вызов run_model без r_pbz — совместимо с любой версией model.py."""
    s = st.session_state
    return run_model(
...
def _recommendations(SI_arr, R_arr, M_pbz):
    ...
    if SI_max > 0.5:
        recs.append(("warn",
            "📊 SI > 0.5 — раствор пересыщен. Необходим регулярный мониторинг химсостава воды." if ru else
            "📊 SI > 0.5 — supersaturated. Regular water chemistry monitoring recommended."))
    if SI_max > 1.5:
        recs.append(("danger",
            "🧪 SI > 1.5 — высокий риск нуклеации. Рекомендуется применение ингибитора (фосфонаты, полиакрилаты)." if ru else
            "🧪 SI > 1.5 — high nucleation risk. Consider scale inhibitor (phosphonates, polyacrylates)."))
    if R_rw >= R_max * 0.5:
        recs.append(("danger",
            "⚠️ Максимальный риск сконцентрирован в призабойной зоне. Приоритет: обработка скважины ингибитором." if ru else
            "⚠️ Max risk near wellbore. Priority: chemical squeeze inhibitor treatment."))
    if dP > 5.0:
        recs.append(("warn",
            f"🔩 Депрессия ΔP = {dP:.1f} МПа высокая. Снижение депрессии уменьшит пересыщение." if ru else
            f"🔩 Drawdown ΔP = {dP:.1f} MPa is high. Reducing drawdown decreases supersaturation."))
    if s.pH > 8.0:
        recs.append(("danger",
            "⚗️ pH > 8 — щелочная среда усиливает риск. Рассмотрите подкисление закачиваемой воды." if ru else
            "⚗️ pH > 8 — alkaline conditions increase risk. Consider injection water acidification."))
    if s.pH < 6.5:
        recs.append(("warn",
            "⚗️ pH < 6.5 — кислая среда. Контроль pH снижает концентрацию CO₃²⁻." if ru else
            "⚗️ pH < 6.5 — acidic. pH management reduces CO₃²⁻ concentration."))
    if s.T > 80:
        recs.append(("warn",
            "🌡️ Высокая температура ускоряет кристаллизацию. Рассмотрите теплоизоляцию НКТ." if ru else
            "🌡️ High temperature accelerates crystallisation. Consider tubing insulation."))
    if M_pbz > 50:
        recs.append(("danger",
            f"💾 Масса осадка в ПЗП за период: {M_pbz:.1f} кг. Планируйте химическую обработку скважины." if ru else
            f"💾 Near-wellbore scale: {M_pbz:.1f} kg over period. Plan chemical wellbore treatment."))
    if L["rock_val"][s.rock_idx] == "carbonate":
        recs.append(("warn",
            "🪨 Карбонатный коллектор: химическое сродство с CaCO₃ снижает барьер нуклеации." if ru else
            "🪨 Carbonate reservoir: affinity with CaCO₃ lowers nucleation barrier. Higher risk."))
    return recs
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with c2:
        ...
        if Pw_v >= Pe_v:
            ...
            dP    = Pe_v - Pw_v
            u_dP  = "МПа" if IS_RU else "MPa"
            st.markdown(f"**{L['dP_lbl']}:** `{dP:.2f} {u_dP}`")
    ...
    with cn:
        if st.button(L["btn_calc"], type="primary", disabled=bool(errors)):
            with st.spinner("Расчёт..." if IS_RU else "Calculating..."):
                st.session_state.results = _run_model_safe()
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    # M_pbz считается здесь, не в model.py
    ...
    <div class="kpi-grid">
      ...
      <div class="kpi-card {kc_m}">
        ...
        <div class="kpi-val" style="color:{col_m}">{M_pbz:.2f}</div>
    ...
    with tab_si:
        ...
        fig.add_hline(y=0.5, line=dict(color="#f59e0b", width=1.0, dash="dot"),
    ...
    with tab_p:
        ...
        fig.add_trace(go.Scatter(
            ...
            hovertemplate=f"{L['r_ax']}: %{{x:.1f}}<br>P: %{{y:.3f}} МПа<extra></extra>",
        ))
    ...
    with tab_v:
        ...
        fig.add_trace(go.Scatter(
            ...
            hovertemplate=f"{L['r_ax']}: %{{x:.1f}}<br>v: %{{y:.3e}} м/с<extra></extra>",
        ))
        fig.update_layout(xaxis_title=L["r_ax"], yaxis_title=L["v_ax"],
                          title=L["chart_v"], yaxis_type="log", **PL)
        st.plotly_chart(fig, use_container_width=True)
    ...
    with tab_r:
        ...
        fig.add_trace(go.Scatter(
            ...
            hovertemplate=f"{L['r_ax']}: %{{x:.1f}}<br>R: %{{y:.4e}}<extra></extra>",
        ))
        fig.add_vrect(
            x0=float(r_arr[0]), x1=float(s.r_pbz),
    ...
    def _ring_color(t):
        ...
        if t < 0.33:
            tt = t / 0.33
            return f"rgba({int(30-4*tt)},{int(58+49*tt)},{int(95-21*tt)},0.88)"
        elif t < 0.66:
            tt = (t - 0.33) / 0.33
            return f"rgba({int(26+154*tt)},{int(107-24*tt)},{int(74-65*tt)},0.88)"
        else:
            tt = (t - 0.66) / 0.34
            return f"rgba({int(180+59*tt)},{int(83-15*tt)},{int(9+59*tt)},0.88)"
    ...
    for rv, Rv, w in zip(r_ds, R_ds, widths):
        ...
        fig_hm.add_trace(go.Barpolar(
            r=[rv] * N_SEC, theta=thetas,
            width=[d_th] * N_SEC, base=[rv - w] * N_SEC,
    ...
    if st.button(L["sens_btn"]):
        with st.spinner("Расчёт..." if IS_RU else "Calculating..."):
            base = dict(
                ...
                m=s.m, rock_type=L["rock_val"][s.rock_idx], k=s.k,
                Q_w=s.Q_w, R_e=s.R_e, r_w=s.r_w, h=s.h,
                P_e=s.P_e, P_w=s.P_w, t=s.t, n_points=80,
            )
        ...
        ax_style = dict(gridcolor="#1e2230", linecolor="#1e2230",
                        zerolinecolor="#2d3748",
                        tickfont=dict(color="#9ca3af", family="DM Sans,sans-serif"))
        fig_s = make_subplots(rows=1, cols=3,
            subplot_titles=[L["sens_pH"], L["sens_T"], L["sens_Pw"]])

        for col_n, (x, y, color) in enumerate(
                [(pH_r, R_pH, "#6c9cf5"), (T_r, R_T, "#a78bfa"), (Pw_r, R_Pw, "#34d399")], 1):
        ...
        for ax in ["xaxis", "yaxis", "xaxis2", "yaxis2", "xaxis3", "yaxis3"]:
            fig_s.update_layout(**{ax: ax_style})
    ...
    if st.button(L["acc_btn"]):
        ...
        # Интегралы по пространству, умноженные на время
        mask_pbz  = r_arr <= max(s.r_pbz, s.r_w * 2)
        int_pbz   = float(np.trapezoid(
            R_arr[mask_pbz] * 2 * math.pi * r_arr[mask_pbz] * s.h,
            r_arr[mask_pbz]
        )) if mask_pbz.sum() >= 2 else 0.0
        int_all   = float(np.trapezoid(
            R_arr * 2 * math.pi * r_arr * s.h, r_arr
        ))

        M_pbz_t = t_pts * int_pbz
        M_all_t = t_pts * int_all

        fig_acc = go.Figure()
        fig_acc.add_trace(go.Scatter(
            x=t_pts / 365, y=M_pbz_t, mode="lines",
            name=L["acc_y_pbz"],
            line=dict(color="#6c9cf5", width=2.5),
            fill="tozeroy", fillcolor="rgba(108,156,245,0.08)",
        ))
        fig_acc.add_trace(go.Scatter(
            x=t_pts / 365, y=M_all_t, mode="lines",
            name=L["acc_y_all"],
    ...
    recs = _recommendations(SI_arr, R_arr, M_pbz)
    if not recs:
    ...
    with st.expander(f"🔎 {L['det_title']}"):
        ...
        with dc1:
            st.metric(L["det_SIe"],  f"{res.get('SI_e', SI_arr[-1]):+.4f}")
            st.metric(L["det_SIrw"], f"{float(SI_arr[0]):+.4f}")
            st.metric(L["det_Rrw"],  f"{float(R_arr[0]):.4e}")
        with dc2:
            st.metric(L["det_Rmax"], f"{R_max:.4e}")
            st.metric(L["det_Mpbz"], f"{M_pbz:.2f} кг" if IS_RU else f"{M_pbz:.2f} kg")
            st.metric(L["det_Mt"],   f"{M_t:.1f} кг"   if IS_RU else f"{M_t:.1f} kg")
            if "eta" in res:
                st.metric(L["det_eta"], f"{res['eta']*100:.2f}%")
